try:
	...
	while (playing):
		...
		if raw_command[0] == 'exit':
			fprint ("@cDo you want to exit the game? (y) ")
			confirm = input()
		...
		if target1 in long_directions:
			target1 = sql.to_short_direction(target1)
	...
	if end_state is None:
		print_all (["You leave the mansion for good, and dead people inside are your concern no more.\n"])
	
	elif end_state == 'out of murderers':
